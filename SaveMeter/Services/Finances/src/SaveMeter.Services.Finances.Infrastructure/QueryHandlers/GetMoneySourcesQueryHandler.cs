...
namespace SaveMeter.Services.Finances.Infrastructure.QueryHandlers
{
    internal class GetMoneySourcesQueryHandler : IQueryHandler<GetMoneySourcesQuery, List<MoneySourceDto>>
    {
        ...
        {
            return await _moneySourceReadRepository.Find(x => !string.IsNullOrEmpty(x.Title)).ProjectToCategoryDto().SortBy(x => x.NativeAmount).ToListAsync(cancellationToken: cancellationToken);
        }
    }
}
